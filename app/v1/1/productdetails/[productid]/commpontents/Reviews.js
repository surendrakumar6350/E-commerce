...
const getProductDetails = useMutation({
  mutationFn: (data) => {
    return axios.post(`${BASE_URL}/api/v1/Reviews/readreviews`, data)
  },
})
useEffect(()=> {
  (
    async () => {
    const ansss = await getProductDetails.mutateAsync({productId: props});
    const all = ansss.data.allReviews;
    all.map((e)=> {
...
{/* review writing box  */}
      <button onClick={reviewbtnhendle} className="inline-block rounded-lg border bg-white px-4 py-2 text-center text-sm font-semibold text-gray-500 outline-none ring-indigo-300 transition duration-100 hover:bg-gray-100 focus-visible:ring active:bg-gray-200 md:px-8 md:py-3 md:text-base">Write a review</button>

      <div id="deleteModal" ref={reff} tabIndex="-1" aria-hidden="true" className="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
    <div className="relative p-4 md:ml-[220px] w-full max-w-md h-full md:h-auto">
        <div className="sm:mt-[80px] mt-[40px] border border-7-black relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            <button onClick={handlecancel} type="button" className="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="deleteModal">
                <svg aria-hidden="true" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd"></path></svg>
                <span className="sr-only">Close modal</span>
            </button>
            <StarRating cancel={handlecancel} props={props}/>
    ...
    <div className="md:w-[100%] md:flex md:flex-wrap  divide-y">
      ...
      <div className={`${data[0] ? null : "hidden"} md:w-[50%] flex flex-col gap-3 py-4 md:py-8`}>
        <div>
          <span className="block text-sm font-bold">{data[0]?.userName}</span>
          <span className="block text-sm text-gray-500">{formatUserFriendlyDate(data[0]?.createdAt)}</span>
        </div>
        ...
        <p className="text-gray-600">{data[0]?.reviewContent}</p>
      </div>
      ...
      <div className={`${data[1] ? null : "hidden"} md:w-[50%] flex flex-col gap-3 py-4 md:py-8`}>
        <div>
          <span className="block text-sm font-bold">{data[1]?.userName}</span>
          <span className="block text-sm text-gray-500">{formatUserFriendlyDate(data[1]?.createdAt)}</span>
        </div>
        ...
        <p className="text-gray-600">{data[1]?.reviewContent}</p>
      </div>
      ...
      <div className={`${data[2] ? null : "hidden"} md:w-[50%] flex flex-col gap-3 py-4 md:py-8`}>
        <div>
          <span className="block text-sm font-bold">{data[2]?.userName}</span>
          <span className="block text-sm text-gray-500">{formatUserFriendlyDate(data[2]?.createdAt)}</span>
        </div>
        ...
        <p className="text-gray-600">{data[2]?.reviewContent}</p>
      </div>
      ...
      <div className={`${data[3] ? null : "hidden"} md:w-[50%] flex flex-col gap-3 py-4 md:py-8`}>
        <div>
          <span className="block text-sm font-bold">{data[3]?.userName}</span>
          <span className="block text-sm text-gray-500">{formatUserFriendlyDate(data[3]?.createdAt)}</span>
        </div>
        ...
        <p className="text-gray-600">{data[3]?.reviewContent}</p>
      </div>
